...
{
	lpc_io_write_byte(value, (unsigned long)p->membase + (offset << p->regshift));
}
...
static unsigned int hs_uart_serial_in(struct uart_port *p, int offset)
{
	...
	value = lpc_io_read_byte((unsigned long)p->membase + (offset << p->regshift));
	return value;
}
...
static int hs_uart_probe(struct platform_device *pdev)
{
	...
	uart.port.mapbase = 0; /* regs->start;*/
	/*
	there is an judgement at kernel function serial8250_console_setup
	if (!port->iobase && !port->membase)
		return -ENODEV;
	so we must set uart.port.membase or uart.port.iobase  non-zero;
	*/
