		      ...
		      unsigned int addr, unsigned char *buf, unsigned int len)
{
	...
	if (HS_LPC_CMD_SAMEADDR_SING != slv_access_mode && HS_LPC_CMD_SAMEADDR_INC
		!= slv_access_mode) {
		dev_err(lpc_dev->dev, "Slv access mode error\n");
		return -EINVAL;
	}

	if ((HS_LPC_CMD_TYPE_IO != cycle_type) && (HS_LPC_CMD_TYPE_MEM != cycle_type)
		&& (HS_LPC_CMD_TYPE_FWH != cycle_type)) {
		     ...
		     unsigned int addr, unsigned char *buf, unsigned int len)
{
	...
	if (HS_LPC_CMD_SAMEADDR_SING != slv_access_mode
		&& HS_LPC_CMD_SAMEADDR_INC != slv_access_mode) {
		dev_err(lpc_dev->dev, "Slv access mode error\n");
		return -EINVAL;
	}

	if (HS_LPC_CMD_TYPE_IO != cycle_type
		&& HS_LPC_CMD_TYPE_MEM != cycle_type
		&& HS_LPC_CMD_TYPE_FWH != cycle_type) {
		dev_err(lpc_dev->dev, "Cycle type error\n");
		return -EINVAL;
	}
